...
1. type() 用于基础类型判断如 int、str
2. isinstance() 用于基础类型判断,内部会检查继承关系
3. dir() 返回一个对象所有属性和方法
...
在测试过程中首先保证产品和需求的一致性,即功能测试满足要求,同时能够使用mysql对数据进行增删改查完成测试中某一状态行为的转变以满足业务流程需要,根据postMan以及charles抓包分析接口以及http请求分析缺陷原因,在发券系统中jenkins + jmeter持续集成压力测试
