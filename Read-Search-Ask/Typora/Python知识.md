...
| 表示层(Presentation)  |                  数据的表示安全和压缩                  |
| 会话层(Session)       |                    会话建立管理终止                    |
| 传输层(Transpot)      |        定义传输数据协议端口号以及流控和差错校验        |
| 网络层(Network)       |       进行逻辑地址寻址实现不同网络之间的路径选择       |
| 数据链路层(Data Link) |      建立逻辑连接进行硬件地址寻址,差错校验等功能       |
| 物理层(Physical)      | 计算机之间二进制信号传输比特流传输建立维护断开物理连接 |
...
| 网络层(Internet)          |      IP ICMP IGMP(IP地址)       |
...
URL 的编码格式采用的是 ASCII 编码,而不是 unicode,即是说所有的非 ASCII 字符都要编码之后再传输
...
在测试过程中首先保证产品和需求的一致性,即功能测试满足要求,同时借助mysql对数据进行增删改查完成测试中某一状态行为的转变以满足业务流程需要,根据postMan以及charles抓包分析接口以及http请求分析缺陷原因,在发券系统中jenkins + jmeter持续集成接口压力测试
...
- jmeter单机接口压力测试.
- 涉及库有json、datetime、openpyxl、pymysql、requests、smtplib、email.mine.multipart
